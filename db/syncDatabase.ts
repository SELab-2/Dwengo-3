...
const contentTypeMap: Map<string, ContentTypeEnum> = new Map();
...
async function fullSyncLearningObjects(prisma: PrismaClient) {
    ...
    for (const [index, remote] of remoteObjects.entries()) {
        const rawContent = await fetchRemoteData(`${API_URLS.learningObjectsContentById}?hruid=${remote.hruid}&version=${remote.version}&language=${remote.language}`);
    ...
    for (const [index, path] of remotePaths.entries()) {
        ...
        for (const [node_index, node] of path.nodes.entries()) {
            for (const transition of node.transitions) {
                ...
                let toNode = await learningObject?.learningPathNodes.find(node => node.learningPathId === path._id);
