...
import {
  ...
  FileSubmission,
} from '../../server/util/types/assignmentSubmission.types';
...
describe('assignmentSubmission persistence test', () => {
  ...
  afterAll(async () => {
    await deleteAllData();
    const count = PrismaSingleton.instance.assignmentSubmission.count();
  ...
  describe('test get assignmentSubmissions', () => {
    test('request with existing groupId responds correctly', async () => {
      for (const submission of submissions) {
        const req = assignemntSubmissionPersistence.getAssignmentSubmissions(
          { groupId: submission.group!.id },
          { page: 1, pageSize: 10, skip: 0 },
        );
        const exptectedSubmissions = submissions
          .filter((sub) => sub.group!.id === submission.group!.id)
          .map((sub) => ({
            id: sub.id,
          }));
        expect(exptectedSubmissions).not.toEqual([]);
        await expect(req).resolves.toStrictEqual({
          data: expect.arrayContaining(exptectedSubmissions),
          totalPages: 1,
        });
      }
    });
    ...
    test('request with existing nodeId and groupId responds correctly', async () => {
      for (const submission of submissions) {
        const req = assignemntSubmissionPersistence.getAssignmentSubmissions(
          { groupId: submission.group!.id, nodeId: submission.node.id },
          { page: 1, pageSize: 10, skip: 0 },
        );
