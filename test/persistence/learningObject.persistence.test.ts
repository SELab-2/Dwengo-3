...
import { LearningObjectDetail } from '../../server/util/types/learningObject.types';
...
describe('learningObject persistence test', () => {
  ...
  describe('test get learningObjects', () => {
    test('request all learning objects', async () => {
      const req = learningObjectPersistence.getLearningObjects(
      ...
      await expect(req).resolves.toEqual({
        data: expect.arrayContaining(expectedObjects),
        totalPages: 1,
      });
    });
    ...
    test('request all learning objects with target ages', async () => {
      ...
      const expectedObjects = learningObjects
        .filter((object) => object.targetAges.includes(10) || object.targetAges.includes(15))
        .map((object) => ({
          id: object.id,
          title: object.title,
          language: object.language,
    ...
    test('request all learing object with keywords', async () => {
      ...
      const expectedObjects = learningObjects
        .filter((object) => object.keywords.some((keyword) => keyword.keyword === 'test4'))
        .map((object) => ({
          id: object.id,
          title: object.title,
          language: object.language,
