...
import { testLearningPathNodes, testPaginationFilter, testTeachers, testStudents, testUsers, testLearningPaths } from '../../testObjects.json';
...
describe('learningPathNode domain', () => {
  ...
  describe('createLearningPathNode', () => {
    test('valid params and user is teacher passes', async () => {
        await expect(learningPathNodeDomain.createLearningPathNode(createLearningPathNodeParams, userTeacher)).resolves.not.toThrow()
    });
    test('user is student fails', async () => {
        await expect(learningPathNodeDomain.createLearningPathNode(createLearningPathNodeParams, userStudent)).rejects.toThrow()
    });
    test('invalid learningpath id fails', async () => {
        await expect(learningPathNodeDomain.createLearningPathNode(createLearningPathNodeInvalidPathIdParams, userTeacher)).rejects.toThrow()
    });
    test('invalid learningobject id fails', async () => {
        await expect(learningPathNodeDomain.createLearningPathNode(createLearningPathNodeInvalidObjectIdParams, userTeacher)).rejects.toThrow()
    });
  });
});