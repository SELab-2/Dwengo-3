...
import { testLearningPathNodes, testLearningPathNodeTransitions, testTeachers, testStudents, testUsers, testLearningPaths } from '../../testObjects.json';
...
describe('learningPathNodeTransition domain', () => {
  ...
  describe('createLearningPathNodeTransition', () => {
    test('valid params and user is teacher passes', async () => {
        await expect(learningPathNodeTransitionDomain.createLearningPathNodeTransition(createLearningPathNodeTransitionParams, userTeacher)).resolves.not.toThrow()
    });
    test('user is student fails', async () => {
        await expect(learningPathNodeTransitionDomain.createLearningPathNodeTransition(createLearningPathNodeTransitionParams, userStudent)).rejects.toThrow()
    });
    test('invalid pathnode id fails', async () => {
        await expect(learningPathNodeTransitionDomain.createLearningPathNodeTransition(createLearningPathNodeTransitionInvalidPathNodeIdParams, userTeacher)).rejects.toThrow()
    });
    test('invalid tonodeindex fails', async () => {
        await expect(learningPathNodeTransitionDomain.createLearningPathNodeTransition(createLearningPathNodeTransitionInvalidNodeIndexParams, userTeacher)).rejects.toThrow()
    });
  });
});