...
import { testClasses, testPaginationFilter, testTeachers, testStudents, testUsers } from '../../testObjects.json';
...
let getClassesTeacherInvalidPaginationQuery = { ...testPaginationFilter, page: '-1', teacherId: testTeachers[0].id };
...
describe('class domain', () => {
    ...
    describe('getClasses', () => {
        ...
        test('teacher id provided and corresponds with user teacher id passes', async () => {
            await expect(classDomain.getClasses(getClassesTeacherQuery, userTeacher)).resolves.not.toThrow()
        });
        test('student id provided and corresponds with user student id passes', async () => {
            await expect(classDomain.getClasses(getClassesStudentQuery, userStudent)).resolves.not.toThrow()
        });
        test('student id provided and user is teacher passes', async () => {
            await expect(classDomain.getClasses(getClassesStudentQuery, userTeacher)).resolves.not.toThrow()
        });
        test('teacher id provided and user is student passes', async () => {
            await expect(classDomain.getClasses(getClassesTeacherQuery, userStudent)).resolves.not.toThrow()
        });
        test('invalid pagination fails', async () => {
            await expect(classDomain.getClasses(getClassesTeacherInvalidPaginationQuery, userTeacher)).rejects.toThrow()
        });
        ...
        test('student id provided and user is student with other id fails', async () => {
            await expect(classDomain.getClasses(getClassesOtherStudentQuery, userStudent)).rejects.toThrow()
        });
        test('teacher id provided and user is teacher with other id fails', async () => {
            await expect(classDomain.getClasses(getClassesOtherTeacherQuery, userTeacher)).rejects.toThrow()
        });
    });
    describe('getClassById', () => {
        ...
        test('student does not belong to class fails', async () => {
            await expect(classDomain.getClassById(getClassByIdUserDoesntBelongId, userStudent)).rejects.toThrow()
        });
        test('teacher does not belong to class fails', async () => {
            await expect(classDomain.getClassById(getClassByIdUserDoesntBelongId, userTeacher)).rejects.toThrow()
        });
    });
    describe('createClass', () => {
        ...
        test('invalid param fails', async () => {
            await expect(classDomain.createClass(createClassInvalidParams, userTeacher)).rejects.toThrow()
        });
    });
    describe('updateClass', () => {
        beforeEach(() => {
            mockClassPeristence.isTeacherFromClass.mockImplementation((userId, classId) => {
                return existingClasses.some(c => c.id === classId && c.teachers.some(t => t.userId === userId));
            });
        });
        ...
        test('invalid id param fails', async () => {
            await expect(classDomain.updateClass(updateClassInvalidIdParams, userStudent)).rejects.toThrow()
        });
        test('invalid name param fails', async () => {
            await expect(classDomain.updateClass(updateClassInvalidNameParams, userTeacher)).rejects.toThrow()
        });
        test('user is not teacher of class fails', async () => {
            await expect(classDomain.updateClass(updateClassTeacherDoesntBelongParams, userTeacher)).rejects.toThrow()
        });
        test('user is student fails', async () => {
