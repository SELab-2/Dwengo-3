...
import { testDiscussions, testPaginationFilter, testTeachers, testStudents, testUsers, testMessages } from '../../testObjects.json';
...
describe('message domain', () => {
    ...
    describe('getMessages', () => {
        ...
        */
        test('invalid pagination fails', async () => {
            await expect(messageDomain.getMessages(getMessagesInvalidPaginationQuery, userStudent)).rejects.toThrow()
        });
        ...
        test('invalid discussion id fails', async () => {
            await expect(messageDomain.getMessages(getMessagesInvalidDiscussionIdQuery, userStudent)).rejects.toThrow()
        });
    });
    describe('createMessage', () => {
        ...
        */
        test('invalid discussion id fails', async () => {
            await expect(messageDomain.createMessage(createMessageInvalidDiscussionIdParams, userStudent)).rejects.toThrow()
        });
        test('invalid content fails', async () => {
            await expect(messageDomain.createMessage(createMessageInvalidContentParams, userStudent)).rejects.toThrow()
        });
    });
    describe('deleteMessage', () => {
        test('valid id passes', async () => {
          await expect(messageDomain.deleteMessage(deleteMessageId, userStudent)).resolves.not.toThrow()
        });
        test('invalid id fails', async () => {
            await expect(messageDomain.createMessage(deleteMessageInvalidId, userStudent)).rejects.toThrow()
        });
        test('message does not belong to user fails', async () => {
            await expect(messageDomain.createMessage(deleteMessageInvalidId, userTeacher)).rejects.toThrow()
        });
    });
});
