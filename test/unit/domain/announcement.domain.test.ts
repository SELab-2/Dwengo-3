...
import { testUsers, testTeachers, testStudents, testPaginationFilter, testClasses, testAnnnouncements } from '../../testObjects.json';
...
describe('announcement domain', () => {
    ...
    describe('getAnnouncements', () => {
        test('valid student query passes', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsStudentQuery, userStudent)).resolves.not.toThrow()
        });
        test('valid teacher query passes', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsTeacherQuery, userTeacher)).resolves.not.toThrow()
        });
        test('invalid pagination fails', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsInvalidPaginationQuery, userTeacher)).rejects.toThrow()
        });
        test('empty query fails', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsEmptyQuery, userTeacher)).rejects.toThrow()
        });
        test('invalid class id fails', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsInvalidClassIdQuery, userTeacher)).rejects.toThrow()
        });
        test('invalid teacher id fails', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsInvalidTeacherIdQuery, userTeacher)).rejects.toThrow()
        });
        test('invalid student id fails', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsInvalidStudentIdQuery, userStudent)).rejects.toThrow()
        });
        test('user does not belong to class fails', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsNotOfClassQuery, userTeacher)).rejects.toThrow()
        });
        test('student id is not user fails', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsStudentNotUserQuery, userStudent)).rejects.toThrow()
        });
        test('teacher id is not user fails', async () => {
            await expect(announcementDomain.getAnnouncements(getAnnouncementsTeacherNotUserQuery, userTeacher)).rejects.toThrow()
        });
    });
    describe('getAnnouncementById', () => {
        test('valid id passes', async () => {
            await expect(announcementDomain.getAnnouncementById(getAnnouncementByIdId, userTeacher)).resolves.not.toThrow()
        });
        test('invalid id fails', async () => {
            await expect(announcementDomain.getAnnouncementById(getAnnouncementByIdInvalidId, userTeacher)).rejects.toThrow()
        });
        test('user does not belong to class fails', async () => {
            await expect(announcementDomain.getAnnouncementById(getAnnouncementByIdNotOfClassId, userTeacher)).rejects.toThrow()
        });
    });
    describe('createAnnouncement', () => {
        test('valid params passes', async () => {
            await expect(announcementDomain.createAnnouncement(createAnnouncementParams, userTeacher)).resolves.not.toThrow()
        });
        test('empty title fails', async () => {
            await expect(announcementDomain.createAnnouncement(createAnnouncementEmptyTitleParams, userTeacher)).rejects.toThrow()
        });
        test('empty content fails', async () => {
            await expect(announcementDomain.createAnnouncement(createAnnouncementEmptyContentParams, userTeacher)).rejects.toThrow()
        });
        test('invalid class id fails', async () => {
            await expect(announcementDomain.createAnnouncement(createAnnouncementInvalidClassIdParams, userTeacher)).rejects.toThrow()
        });
        test('user is not teacher fails', async () => {
            await expect(announcementDomain.createAnnouncement(createAnnouncementParams, userStudent)).rejects.toThrow()
        });
        test('user does not belong to class fails', async () => {
            await expect(announcementDomain.createAnnouncement(createAnnouncementNotOfClassParams, userTeacher)).rejects.toThrow()
        });
    });
    describe('updateAnnouncement', () => {
        test('valid params passes', async () => {
            await expect(announcementDomain.updateAnnouncement(updateAnnouncementId, updateAnnouncementParams, userTeacher)).resolves.not.toThrow()
        });
        test('empty title fails', async () => {
            await expect(announcementDomain.updateAnnouncement(updateAnnouncementId, updateAnnouncementEmptyTitleParams, userTeacher)).rejects.toThrow()
        });
        test('empty content fails', async () => {
            await expect(announcementDomain.updateAnnouncement(updateAnnouncementId, updateAnnouncementEmptyContentParams, userTeacher)).rejects.toThrow()
        });
        test('user is not teacher fails', async () => {
            await expect(announcementDomain.updateAnnouncement(updateAnnouncementId, updateAnnouncementParams, userStudent)).rejects.toThrow()
        });
