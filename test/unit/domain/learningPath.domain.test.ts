...
import { testClasses, testPaginationFilter, testTeachers, testStudents, testUsers, testLearningPaths } from '../../testObjects.json';
...
describe('learningPath domain', () => {
  ...
  describe('getLearningPaths', () => {
    test('valid query passes', async () => {
      await expect(learningPathDomain.getLearningPaths(getLearningPathsQuery)).resolves.not.toThrow()
    });
    test('invalid pagination fails', async () => {
      await expect(learningPathDomain.getLearningPaths(getLearningPathsInvalidPaginationQuery)).rejects.toThrow()
    });
    test('invalid keywords fails', async () => {
      await expect(learningPathDomain.getLearningPaths(getLearningPathsInvalidKeywordsQuery)).rejects.toThrow()
    });
    test('invalid age fails', async () => {
      await expect(learningPathDomain.getLearningPaths(getLearningPathsInvalidAgeQuery)).rejects.toThrow()
    });
  });
  describe('createLearningObject', () => {
    test('valid params and user is teacher passes', async () => {
      await expect(learningPathDomain.createLearningPath(createLearningPathParams, userTeacher)).resolves.not.toThrow()
    });
    test('user is student fails', async () => {
      await expect(learningPathDomain.createLearningPath(createLearningPathParams, userStudent)).rejects.toThrow()
    });
    test('invalid hruid fails', async () => {
        await expect(learningPathDomain.createLearningPath(createLearningPathInvalidHruidParams, userTeacher)).rejects.toThrow()
    });
    test('invalid language fails', async () => {
        await expect(learningPathDomain.createLearningPath(createLearningPathInvalidLanguageParams, userTeacher)).rejects.toThrow()
    });
    test('invalid title fails', async () => {
        await expect(learningPathDomain.createLearningPath(createLearningPathInvalidTitleParams, userTeacher)).rejects.toThrow()
    });
  });
});