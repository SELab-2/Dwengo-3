...
import { testTeachers, testUsers, testStudents, testClasses, testPaginationFilter } from '../../testObjects.json';

import { LoginRequest, RegisterRequest } from '../../../server/util/types/RequestTypes';
...
describe('teacher domain', () => {
    ...
    describe('getTeachers', () => {
        test('valid pagination and no params passes', async () => {
            await expect(teacherDomain.getTeachers(getTeachersEmptyQuery, userTeacher)).resolves.not.toThrow();
        });
        ...
        test('invalid pagination fails', async () => {
            await expect(teacherDomain.getTeachers(getTeachersInvalidPaginationQuery, userTeacher)).rejects.toThrow();
        });
        test('invalid user id param fails', async () => {
            await expect(teacherDomain.getTeachers(getTeachersInvalidUserIdQuery, userTeacher)).rejects.toThrow();
        });
        test('invalid class id param fails', async () => {
            await expect(teacherDomain.getTeachers(getTeachersInvalidClassIdQuery, userTeacher)).rejects.toThrow();
        });
        test('invalid classes include param fails', async () => {
            await expect(teacherDomain.getTeachers(getTeachersInvalidIncludeQuery, userTeacher)).rejects.toThrow();
        });
    });
    describe('updateTeacher', () => {
        test('valid id and class id params passes', async () => {
            await expect(teacherDomain.updateTeacher(updateTeacherParams1, userTeacher)).resolves.not.toThrow();
        });
        test('valid id and no class ids params passes', async () => {
            await expect(teacherDomain.updateTeacher(updateTeacherParams2, userTeacher)).resolves.not.toThrow();
        });
        test('invalid id params fails', async () => {
            await expect(teacherDomain.updateTeacher(updateTeacherInvalidIdParam, userTeacher)).rejects.toThrow();
        });
        test('invalid classes params fails', async () => {
            await expect(teacherDomain.updateTeacher(updateTeacherInvalidClassesParam, userTeacher)).rejects.toThrow();
        });
    });
    describe('deleteTeacher', () => {
        test('valid id passes', async () => {
            await expect(teacherDomain.deleteTeacher(deleteTeacherParams, userTeacher)).resolves.not.toThrow();
        });
        test('invalid id fails', async () => {
            await expect(teacherDomain.deleteTeacher(deleteTeacherInvalidParams, userTeacher)).rejects.toThrow();
        });
    });
});