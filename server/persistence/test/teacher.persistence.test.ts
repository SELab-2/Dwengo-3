    ...
    beforeAll(async () => {
        ...
        for (const classData of classesDetail) {
            for (const teacher of classData.teachers) {
                teacherPromises.push(getUserById(teacher.userId));
            }
    ...
    describe("test get teachers", () => {
        test("request all teachers with classId", async () => {
            ...
            for (const classData of classes) {
                const req = teacherPersistence.getTeachers({page: 1, pageSize: 10, skip: 0}, {classId: classData.id});
                await expect(req).resolves.toEqual({data: expect.arrayContaining(expectedTeachers), totalPages: 1});
            }
        });
    });
});