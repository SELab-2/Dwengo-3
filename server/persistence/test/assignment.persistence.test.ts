...
describe("assignment persistence test", () => {
    ...
    describe("test get assignments", () => {
        test("request with existing classId responds correctly", async () => {
            for (const assignment of assignments) {
                const req = assignmentPersistence.getAssignments({classId: assignment.class.id}, {page: 1, pageSize: 10, skip: 0});
                const expectedAssignments = assignments.filter((ass) => ass.class.id === assignment.class.id).map((ass) => ({
                    id: ass.id,
                    learningPathId: ass.learningPath.id
                }));
                expect(expectedAssignments).not.toEqual([]);
                await expect(req).resolves.toEqual({data: expect.arrayContaining(expectedAssignments), totalPages: 1});
            }
        });
        ...
        test("request with existing groupId responds correctly", async () => {
            for (const assignment of assignments) {
                for (const group of assignment.groups) {
                    const req = assignmentPersistence.getAssignments({groupId: group.id}, {page: 1, pageSize: 10, skip: 0});
        ...
        test("request with existing teacherId responds correctly", async () => {
            ...
            for (const assignment of assignments) {
                ...
                for (const teacher of classData.teachers) {
                    const req = assignmentPersistence.getAssignments({teacherId: teacher.id}, {page: 1, pageSize: 10, skip: 0});
                    await expect(req).resolves.toEqual({data: expect.arrayContaining(expectedAssignments), totalPages: 1});
                }
            }
        });
        ...
        test("request with existing studentId responds correctly", async () => {
            ...
            for (const assignment of assignments) {
                ...
                for (const student of classData.students) {
                    const req = assignmentPersistence.getAssignments({studentId: student.id}, {page: 1, pageSize: 10, skip: 0});
                    await expect(req).resolves.toEqual({data: expect.arrayContaining(expectedAssignments), totalPages: 1});
                }
            }
        });
