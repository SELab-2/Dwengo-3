...
import { MessageDetail } from "../../util/types/message.types";
...
describe("message persitence test", () => {
    ...
    describe("test get messages", () => {
        test("request with existing discussionId responds correctly", async () => {
            for (const message of messages) {
                const req = messagePersistence.getMessages({discussionId: message.discussionId}, {page: 1, pageSize: 10, skip: 0});
                const expectedMessages = messages.filter((m) => m.discussionId === message.discussionId).map((m) => ({
                    id: m.id,
                    content: m.content,
                    sender: m.sender,
                ...
                await expect(req).resolves.toStrictEqual({data: expect.arrayContaining(expectedMessages), totalPages: 1});
            }
        });
    });
});