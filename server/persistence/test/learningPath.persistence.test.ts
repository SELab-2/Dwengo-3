...
describe("learningPath persistence test", () => {
    ...
    describe("test get learningPaths", () => {
        test("request all learning paths", async () => {
            ...
            await expect(req).resolves.toEqual({data: expect.arrayContaining(expectedPaths), totalPages: 1});
        });

        test("request all learning paths with target ages", async () => {
            const req = learningPathPersistence.getLearningPaths({age: [10]}, {page: 1, pageSize: 10, skip: 0});
            const expectedPaths = learningPaths.filter((path) => 
                path.learningPathNodes.some((node) => node.learningObject.targetAges.includes(10))).map((path) => ({
                id: path.id,
                title: path.title,
                learningPathNodes: expect.arrayContaining(path.learningPathNodes),
                image: path.image,
        ...
        test("request all learning paths with target ages", async () => {
            const req = learningPathPersistence.getLearningPaths({keywords: ["test4"]}, {page: 1, pageSize: 10, skip: 0});
            const expectedPaths = learningPaths.filter((path) => 
                path.learningPathNodes.some((node) => node.learningObject.keywords.some((keyword) => keyword.keyword === "test4"))).map((path) => ({
                id: path.id,
                title: path.title,
                learningPathNodes: expect.arrayContaining(path.learningPathNodes),
                image: path.image,
