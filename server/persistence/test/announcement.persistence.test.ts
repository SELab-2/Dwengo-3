...
import { AnnouncementDetail } from "../../util/types/announcement.types";
...
describe("announcement persistence test", () => {
    ...
    describe("test check announcement is from teacher", () => {
        test("request with existing teacher id responds with nothing", async () => {
            for (const announcement of announcements) {
                const req = announcementPersistence.checkAnnouncementIsFromTeacher(announcement.id, announcement.teacher.id);
                await expect(req).resolves.not.toThrow();
            }
        });

        test("request with unexisting teacher id responds with an error", async () => {
            const req = announcementPersistence.checkAnnouncementIsFromTeacher(announcements[0].id, "qlskfqlfmjq");
            await expect(req).rejects.toThrow();
        });
    });
    ...
    describe("test get announcemnts", () => {
        test("request with existing classId responds correctly", async () => {
            for (const announcement of announcements) {
                const req = announcementPersistence.getAnnouncements({classId: announcement.class.id}, {page: 1, pageSize: 10, skip: 0});
                const expectedAnnouncements = announcements.filter((ann) => ann.class.id === announcement.class.id).map((ann) => ({
                    id: ann.id,
                    title: ann.title
                }));
                expect(expectedAnnouncements).not.toEqual([]);
                await expect(req).resolves.toEqual({data: expect.arrayContaining(expectedAnnouncements), totalPages: 1});
            }
        });
        ...
        test("request with existing teacherId responds correctly", async () => {
            for (const announcement of announcements) {
                ...
                for (const teacher of classData.teachers) {
                    const req = announcementPersistence.getAnnouncements({teacherId: teacher.id}, {page: 1, pageSize: 10, skip: 0});
        ...
        test("request with existing studentId responds correctly", async () => {
            for (const announcement of announcements) {
                ...
                for (const student of classData.students) {
                    const req = announcementPersistence.getAnnouncements({studentId: student.id}, {page: 1, pageSize: 10, skip: 0});
                    const expectedAnnouncements = announcements.map((ann) => ({
                        id: ann.id,
                        title: ann.title
                    }));
    ...
    describe("test update announcement", async () => {
        test("request with existing id should update announcemnent correctly", async () => {
            for (const announcement of announcements) {
                ...
                const req = announcementPersistence.updateAnnouncement(announcement.id, {title: announcement.title, content: announcement.content});
                await expect(req).resolves.toStrictEqual(announcement);
