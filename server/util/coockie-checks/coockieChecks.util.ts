...
export const compareUserIdWithFilterId = async (user: UserEntity, studentId: Uuid | undefined, teacherId: Uuid | undefined): Promise<void> => {
    if (
...
export const checkIfUserIsInClass = async (user: UserEntity, classId: Uuid | undefined, classPersistance: ClassPersistence): Promise<void> => {
...
export const checkIfUserIsInGroup = async (user: UserEntity, groupId: Uuid | undefined, groupPersistence: GroupPersistence): Promise<void> => {
    if (! groupId) return;
...
export const checkIfUsersAreInSameClass = async (groups: Uuid[][], classId: Uuid, teacherId: Uuid, classPersistance: ClassPersistence): Promise<void> => {
    const classData = await classPersistance.getClassById(classId);
    if (!classData) {
        throw new Error("Class not found.");
    }
...
export const checkIfUsersAreInSameGroup = async (users: Uuid[], groupId: Uuid, groupPersistence: GroupPersistence): Promise<void> => {
  const groupData = await groupPersistence.getGroupById(groupId);
  if (!groupData) {
