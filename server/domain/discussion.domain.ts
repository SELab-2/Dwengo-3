...
export class DiscussionDomain {
    ...
    public async getDiscussions(query: any): Promise<{data: Discussion[], totalPages: number}> {
        const parseResult = queryWithPaginationParser(query, DiscussionFilterSchema);
        return this.discussionPersistence.getDiscussions(parseResult.dataSchema, parseResult.dataPagination);
    }
